...
const WYSIWYGOptionEditor = ({questionIndex, index, option, handleQuestionOptionChange }) => {
  ...
  const editor = useEditor({
    ...
    content: typeof window !== "undefined" ? option : "", // Ensure content is loaded only on the client
    editorProps: {
      attributes: {
        class:
  ...
  useEffect(() => {
    if (editor) {
      const handleEditorChange = () => {
        handleQuestionOptionChange(questionIndex, index, editor.getHTML());
      };
  ...
  }, [editor, index, handleQuestionOptionChange]);
...
export default WYSIWYGOptionEditor;
